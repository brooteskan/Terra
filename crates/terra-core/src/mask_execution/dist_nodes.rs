//! Context-aware distribution-node evaluation.

use crate::heightfield::{Heightfield, HeightfieldMetrics};
use crate::mask_field::MaskField;
use crate::mask_ir::{DistNode, DistNodeKind, MaskCombine};
use crate::mask_types::{MaskId, MaskRef};
use crate::noise::{perlin2, worley2, WorleyMetric};
use crate::spatial_kernels::{dist_point_segment, point_in_polygon};
...
fn eval_node_base(
    ...
    match &node.kind {
        ...
        DistNodeKind::Occlusion { radius, strength } => {
            ...
            crate::spatial_kernels::ambient_occlusion(hf, *radius, *strength)
        }
...
mod tests {
    use super::*;
    use crate::mask_ir::ClimateMaskChannel;
    ...
    fn occlusion_uses_ambient_occlusion_when_height_present() {
        ...
        let expected = crate::spatial_kernels::ambient_occlusion(&hf, 2, 1.0);
        for j in 0..metrics.height {
            for i in 0..metrics.width {
