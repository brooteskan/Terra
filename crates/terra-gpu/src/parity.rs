...
/// bit-exact `hash_noise` port keeps Noise strokes in the same budget; Smooth's
/// base-3x3 pull (#114) is transcendental-free (nine same-order adds and a divide),
/// so it does not widen the worst case. Measured worst case is ~6.5e-5 m over the
/// 13-kind stroke set; this holds portable headroom.
...
| `authoring.sculpt-strokes` | Per-sample stroke kinds, Smooth, and distance stamps, supported blend/mask | 0.001 | 0.00001 |\n\
