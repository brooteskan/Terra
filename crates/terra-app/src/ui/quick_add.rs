...
// Variants ultimately carry the intrinsic-size `LayerKind` editor payload; picker items
// are transient UI rows, so the variance is accepted rather than boxing every match site.
