...
use super::compiled_plan_support::{
    bridge_plan_boundary, layer_input_independent, owner_layer_id, plan_fallback_diagnostic,
    plan_fallback_result, plan_operation_fallback, resample_bridge_prefix,
};
use super::*;
    ...
    pub(super) fn record_compiled_operation(
        ...
        match &operation.kind {
            ...
            TerrainOpKind::RunLayerKernel {
                ...
                let sculpt_source_guard = if let LayerKind::SculptStrokes(params) = &authored.kind {
                    Some(sculpt_source_guard(params))
                } else {
                    None
                };
                let input_region = sculpt_source_guard.map_or(region, |guard| {
                    expand_sample_region(region, guard, self.metrics.width, self.metrics.height)
                });
                let input_view = if sculpt_source_guard.is_some() {
                    &self.pong.view
                } else {
                    &self.ping.view
                };
                ...
                self.current = u8::from(sculpt_source_guard.is_some());
            ...
            let execution = (|| -> Result<(), CompiledDispatchError> {
                match &operation.kind {
                    ...
                    TerrainOpKind::RunLayerKernel {
                        ...
                        let sculpt_source_guard =
                            if let LayerKind::SculptStrokes(params) = &authored.kind {
                                Some(sculpt_source_guard(params))
                            } else {
                                None
                            };
                        let input_region = sculpt_source_guard.map_or(region, |guard| {
                            expand_sample_region(region, guard, metrics.width, metrics.height)
                        });
                        let input_view = if sculpt_source_guard.is_some() {
                            &self.pong.view
                        } else {
                        ...
                        record_copy_views_region(
                            ...
                            // warm local sculpt execution refreshes only the true
                            // layer input through its private read guard; the guard
                            // is consumed internally and is not published below.
                            input_region,
                        );
                        self.current = u8::from(sculpt_source_guard.is_some());
